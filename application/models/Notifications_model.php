...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        $sql = "SELECT $users_table.id, $users_table.email, $users_table.enable_web_notification, $users_table.enable_email_notification, $users_table.user_type, $users_table.is_admin, $users_table.role_id,
                ...
                WHERE $users_table.deleted=0 AND $users_table.status='active'  AND ($users_table.enable_web_notification=1 OR $users_table.enable_email_notification =1 )  AND (1=2 $where) $extra_where";
        ...
        $senderInfo = $this->Users_model->get_one($user_id);
        //$actionInfo = $this->Users_model->get_one($to_user_id);

        $notificationHead = $user_id ? $senderInfo->first_name.' '.$senderInfo->last_name : get_setting("app_title");
        $notificationBody = lang("notification_" . $event);
        $notificationImage = '';

        $receivers = explode(',', $web_notify_to);
        $url = get_uri("notifications");
        
        foreach($receivers as $receiver)
        {
            send_onesignal ($notificationHead.' has '.$notificationBody, $url, $receiver, $notification_id, '');
        }
