...
class Activity_logs_model extends CI_Model
{
    ...
    function get_logs($options = array())
    {
        $activity_logs_table = $this->db->dbprefix('activity_logs');
        $users_table = $this->db->dbprefix('users');

        $where = '';
        $module = get_array_value($options, "module");
        if ($module) {
            $where .= " AND $activity_logs_table.log_type = '$module'";
        }

        $start_date = get_array_value($options, "start_date");
        $end_date = get_array_value($options, "end_date");
        if ($start_date && $end_date) {
            $where .= " AND ($activity_logs_table.created_at BETWEEN '$start_date' AND '$end_date') ";
        }
        ...
        $sql = "SELECT $activity_logs_table.*,  CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_user, $users_table.image as created_by_avatar
        FROM $activity_logs_table
        LEFT JOIN $users_table ON $users_table.id= $activity_logs_table.created_by

        WHERE $activity_logs_table.deleted=0 $where 
        ORDER BY $activity_logs_table.created_at DESC";
