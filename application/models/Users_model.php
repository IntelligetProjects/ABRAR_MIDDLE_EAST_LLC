...
class Users_model extends Crud_model
{
    ...
    {
        $this->table = 'users';
        parent::__construct($this->table, true/* has cost center*/);
    }
    ...
    function get_details($options = array())
    {
        ...
        if ($payroll) {
            $where .= " AND $users_table.payroll=$payroll";
        }
        if ($status === "active") {
        ...
        if ($allow_branch_select) {
            $where .= " AND $users_table.allow_branch_select='$allow_branch_select'";
        }
        ...
        $where .= " AND $users_table.is_super=0";
        ...
        $sql = "SELECT $users_table.*,
            ...
            $team_member_job_info_table.housing, 
            $team_member_job_info_table.transportation, 
            $team_member_job_info_table.telephone, 
            $team_member_job_info_table.utility, 
            $team_member_job_info_table.national, 
            $team_member_job_info_table.pasi, 
            $team_member_job_info_table.salary_term,
            $team_member_job_info_table.bank_title,
            $team_member_job_info_table.account_title,
            $team_member_job_info_table.account_no
            $select_custom_fieds
        FROM $users_table
    ...
    function get_full_job_info($user_id)
    {
        $job_info_table = $this->db->dbprefix('team_member_job_info');

        $sql = "SELECT $job_info_table.* FROM $job_info_table   
        WHERE $job_info_table.deleted=0 AND $job_info_table.user_id='$user_id'";
    ...
    function get_access_info($user_id = 0)
    {
        ...
        $sql = "SELECT $users_table.id, $users_table.user_type, $users_table.is_admin, $users_table.is_super, $users_table.role_id, $users_table.email,
        ...
        $users_table.is_primary_contact, $users_table.sticky_note, $users_table.cost_center_id,
    ...
    function total_leaves($user_id, $options)
    {
        ...
        $sql = "SELECT  SUM($leave_applications_table.total_hours) AS total_hours,
                SUM($leave_applications_table.total_days) AS total_days, MAX($leave_applications_table.applicant_id) AS applicant_id, $leave_applications_table.status, $leave_applications_table.paid_unpaid
            FROM $leave_applications_table       
            WHERE $leave_applications_table.deleted=0 AND $leave_applications_table.status='approved' AND $leave_applications_table.applicant_id = $user_id $where";
        return $this->db->query($sql)->row();
    }
}
