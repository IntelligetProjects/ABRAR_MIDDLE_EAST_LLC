    ...
    function get_details($options = array())
    {
        ...
        if ($start_date && $deadline) {
            ...
            if ($deadline === "expired") {
                ...
                $where .= " AND (($tasks_table.deadline IS NOT NULL AND $tasks_table.deadline='$deadline') )";
            }
        }
        ...
        // to show tasks unrelated with a project
        $extra_left_join = "";
        $project_member_id = get_array_value($options, "project_member_id");
        if ($project_member_id) {
            $where .= " AND ($project_members_table.user_id=$project_member_id OR $projects.id = 1)";
            $extra_left_join = " LEFT JOIN $project_members_table ON $tasks_table.project_id= $project_members_table.project_id AND $project_members_table.deleted=0 AND ($project_members_table.user_id=$project_member_id OR $projects.id = 1)";
        }
