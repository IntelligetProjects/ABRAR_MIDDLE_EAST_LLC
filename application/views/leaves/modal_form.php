...
<input name="valid" type="hidden" value="0" id="valid">
<div class="modal-body clearfix">
    ...
    <?php if( $can_manage_application){?>
    <div class="form-group">
        <label for="paid_unpaid" class=" col-md-3"><?php echo lang('paid_unpaid_leave'); ?></label>
        <div class=" col-md-9">
            <?php
            echo form_dropdown("paid_unpaid", isset($paid_unpaid_dropdown)?$paid_unpaid_dropdown:'', "", "class='select2 validate-hidden' id='paid_unpaid' data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
            ?>
        </div>
    </div>
    <?php } ?>
    ...
    <div id="single_day_section"  class=" form-group date_section">
        ...
        <div class="col-md-9">
            <input type="datetime-local" name="single_date" class="form-control" pleaceholder="<?php echo lang('date') ?>" required>
            <?php
    ...
    <div class="form-group" id="remaining" style="display:none">
        <label for="reason" class=" col-md-3">Remaining</label>
        <div class=" col-md-9">
        <input type="text" name="remaining" value="" readonly class="form-control">
        </div>
    </div>
</div>
...
<div class="modal-footer">
    ...
    <button type="submit" id="check" class="btn btn-primary"><span class="fa fa-check-circle"></span> Check Remaining</button>
</div>
<?php echo form_close(); ?>
        ...
        $("#leave-form").appForm({
            ...
            onError: function (result){
                ...
                if(result.remaining>0){
                    ...
                    $('#valid').val(1);
                    $('#remaining input').val(result.remaining);
                    $('#remaining').css('display','block');
                    $('#check').text('<?php echo $can_manage_application? lang('assign_leave'): lang('apply_leave')?>');
                    appAlert.success('There is '+result.remaining+' available remaining leave', {duration: 10000});
                }else{
                    $('#app-loader').css('display','none')
                    $('#remaining input').val(result.remaining);
                    $('#remaining').css('display','block'); 
                    appAlert.error('No available remaining leave', {duration: 10000});
                }
            }
        });

        // $("#leave-form .select2").change(function () {
        $("input").change(function () {
            $('#valid').val(0);
            $('#check').text('Check Leave');
