...
<div id="page-content" class="clearfix p20">
    <?php if ($this->login_user->role_id==1 ||$this->login_user->is_admin) { ?>
        <?php echo form_open(get_uri("pending_transactions/enteries_save"), array("id" => "todo-inline-form", "class" => "", "role" => "form")); ?>
...
<script type="text/javascript">
    ...
    updateValidStatus = function (transactionId) {
        $.ajax({
            url: "<?php echo get_uri("pending_transactions/update_valid_status"); ?>/" + transactionId,
    ...
    saveBank = function (transactionId, accId) {
        $.ajax({
            url: "<?php echo get_uri("pending_transactions/save_bank_account"); ?>/" + transactionId + "/" + accId,
            success: function (result) {
                console.log(result);
                if (result) {
...
<script type="text/javascript">
    ...
    $("#entries-table").appTable({
        source: '<?php echo_uri("pending_transactions/enteries_list_data/" . $transactions_info->id . "/") ?>',
        displayLength: 100,
        hideTools: false,
        stateSave: false,
    ...
    updateValidStatus = function (transactionId) {
        $.ajax({
            url: "<?php echo get_uri("pending_transactions/pdate_valid_status"); ?>/" + transactionId,
            success: function (result) {
                if (result) {
                    $("#valid_status").html(result);
                }
    ...
    function applySelect2OnItemTitle() {
        $("#account").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("pending_transactions/get_accounts_suggestion_enteries"); ?>",
                dataType: 'json',
                quietMillis: 250,
