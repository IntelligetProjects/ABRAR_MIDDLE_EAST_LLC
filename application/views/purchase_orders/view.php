    ...
    <div style="max-width: 1000px; margin: auto;">
        <div class="page-title clearfix mt15">
            ...
            <div class="title-button-group">
                ...
                <?php echo modal_anchor(get_uri("purchase_orders/item_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_item'), array("class" => "btn btn-default", "title" => lang('add_item'), "data-post-purchase_order_id" => $purchase_order_info->id)); ?>
                <?php echo modal_anchor(get_uri("purchase_orders/items_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_items'), array("class" => "btn btn-default", "title" => lang('add_items'), "data-post-purchase_order_id" => $purchase_order_info->id)); ?>
                <?php } ?>
                <?php if ($can_add_payment) { ?>
                <?php echo modal_anchor(get_uri("purchase_order_payments/payment_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment'), array("class" => "btn btn-default", "title" => lang('add_payment'), "data-post-purchase_order_id" => $purchase_order_info->id)); ?>
                <?php } ?>
                <?php if ($shipment_status !== "delivered" && $approval_status == 'approved') { ?>
                <?php echo modal_anchor(get_uri("shipments/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_shipment'), array("class" => "btn btn-default", "title" => lang('add_shipment'), "data-post-purchase_order_id" => $purchase_order_info->id)); ?>
                <?php } ?>
                <span class="dropdown inline-block mt10" style="margin-right:36px; margin-top: 10px;">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        ...
                        <?php if ($purchase_order_status == "draft") { ?>
                            <li role="presentation"><?php echo ajax_anchor(get_uri("purchase_orders/update_purchase_order_status/" . $purchase_order_info->id . "/not_paid"), "<i class='fa fa-close'></i> " . lang('mark_as_not_paid'), array("data-reload-on-success" => "1")); ?> </li>
                        <?php } ?>
        ...
        <div class="mt15">
            <div class="panel panel-default p15 b-t">
                ...
                <div class="clearfix">
                    <div class="pull-right pr15" id="purchase_order-total-section">
                        <?php $this->load->view("purchase_orders/purchase_order_total_section", array("purchase_order_id" => $purchase_order_info->id)); ?>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $("#approved").click(function () {
        appLoader.show();
        $.ajax({
            url: "<?php echo get_uri('purchase_orders/update_approval_status/' . $purchase_order_info->id . '/approved') ?>",
            dataType: 'json',
            success: function (result) {
                if (result.success) {
                    location.reload();
