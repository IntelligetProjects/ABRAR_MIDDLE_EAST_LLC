...
<?php echo form_open(get_uri("proforma_invoice_payments/save_payment"), array("id" => "invoice-payment-form", "class" => "general-form", "role" => "form")); ?>
...
<div class="modal-body clearfix">
    ...
    <div class="form-group" id="bank_sec">
        <label for="bank" class=" col-md-3"><?php echo lang('bank'); ?></label>
        <div class="col-md-9">
            <?php
            echo form_dropdown("bank", $banks, array($model_info->bank), "class='select2 validate-hidden' id='bank'");
            ?>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        <?php if ($model_info->payment_method_id == 4) { ?>
            $("#cheque").hide();
            $("#bank_sec").show();
        <?php } else if ($model_info->payment_method_id == 5) { ?>
            $("#cheque").show();
            $("#bank_sec").hide();
        <?php } else { ?>
            $("#cheque").hide();
