<?php echo form_open(get_uri("proforma_invoices/save"), array("id" => "invoice-form", "class" => "general-form", "role" => "form")); ?>
...
<?php if ($can_depart) { ?>
    <div class="form-group" style="display:none">
            ...
            <div class="col-md-9">
                <?php               
                echo form_dropdown("department", $departments, $model_info->department ? array($model_info->department) : array($default_department) , "class='select2 validate-hidden' id='department' data-rule-required='false', data-msg-required='" . lang('field_required') . "'");
                ?>
            </div>
        </div>
    <?php } else { ?>
        <input type="hidden" name="department" value="<?php echo $default_department; ?>" />
    <?php } ?>
    ...
    <div class="form-group">
        ...
        <div class=" col-md-9">
            <?php
            $default_note = ($default_department!= 1 && !empty($default_department)) ? get_setting('invoice_note'.$default_department) : get_setting('invoice_note');
            echo form_textarea(array(
                "id" => "invoice_note",
                "name" => "invoice_note",
                "value" => $model_info->note ? $model_info->note : $default_note,
                "class" => "form-control",
    ...
    <div class="form-group">
        <label for="owner" class=" col-md-3"><?php echo 'owner'; ?></label>
        <div class="col-md-9">
            <?php
            echo form_dropdown("owner", $members_dropdown, $model_info->owner ? $model_info->owner : $client_owner, "class='select2 validate-hidden' id='owner', data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
            ?>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#invoice-form").appForm({
            onSuccess: function (result) {
                if (typeof RELOAD_VIEW_AFTER_UPDATE !== "undefined" && RELOAD_VIEW_AFTER_UPDATE) {
                    ...
                    window.location = "<?php echo site_url('proforma_invoices/view'); ?>/" + result.id;
                }
            },
        ...
        if (!"<?php echo $this->login_user->is_admin; ?>") {
            $("#invoice_bill_date").prop("disabled", true);
        }
