...
class MY_Controller extends CI_Controller {
    ...
    protected function project_permission(){
    ...
    protected function get_clients_and_leads_dropdown($return_json = false) {
        ...
        foreach ($clients as $client) {
            $company_name = $client->is_lead ? lang("lead") . ": " . $client->company_name : $client->company_name." - ".$client->phone;
    ...
    ///// new permission functions
         //initialize the login user's permissions with readable format
    protected function permission_checker($module) {
        $info = $this->get_access($module);
        $this->access_type = $info->access_type;
        $this->allowed_members = $info->allowed_members;
        $this->module_group = $info->module_group;
    }
    // custom check 
    protected function p_check($module){
        if ($this->login_user->is_admin) {
            return true;
        }elseif(get_array_value($this->login_user->permissions, $module)){
            return true;
        }else{
            return false;
    ...
    // custom access check 
    protected function p_access_check($module){
        if ($this->login_user->is_admin) {
            return true;
        }elseif(get_array_value($this->login_user->permissions, $module)){
            return true;
    ...
    protected function module_access_check($module){
        echo $module;
        echo get_setting($module); die('hi');
        if(get_setting($module) == "1"){
            return true;
       }else{
            redirect("forbidden");
        }
    ...
    //prepear the login user's permissions
    protected function get_access($group) {
        $info = new stdClass();
        $info->access_type = "";
        $info->allowed_members = array();
        $info->module_group = $group;
    ...
    //access only allowed team members
    protected function access_allowed_members() {
        if ($this->access_type === "1") {
            return true; //can access if user has permission
        } else {
            $user_id = $this->login_user->id;
            $dashboard_id = get_setting("user_".$user_id."_dashboard");
            if($dashboard_id)
            {
                redirect("dashboard/view/".$dashboard_id);
            }
            else
            {
                redirect("dashboard");    
    ...
    //access only allowed team members or client contacts 
    protected function access_allowed_members_or_client_contact($client_id) {
    ...
    //allowed team members and clint himself can access  
    protected function access_allowed_members_or_contact_personally($user_id) {
        if (!($this->access_type === "1" || $user_id === $this->login_user->id)) {
            redirect("forbidden");
        }
    }
