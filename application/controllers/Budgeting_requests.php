...
class Budgeting_requests extends MY_Controller {

    function __construct() {
        parent::__construct();
        $this->permission_checker("can_access_estimatea");
    }
    ...
    function index() {
        ...
        $this->template->rander('budget_requests/index', $view_data);
    }

    //view estimate request
    function view_estimate_request($id = 0) {

        $model_info = $this->Budgeting_requests_model->get_details(array("id" => $id))->row();

        if ($model_info) {
            $this->access_allowed_members_or_client_contact($model_info->client_id);
        } else {
            show_404();
        }
        ...
        $view_data["estimates"] = $this->Budgeting_model->get_all_where(array("estimate_request_id" => $id))->result();

        $this->template->rander('budgeting_requests/view_estimate_request', $view_data);
    }

    // download files 
    function download_estimate_request_files($id = 0) {
        $this->access_allowed_members();
        $info = $this->Budgeting_requests_model->get_one($id);
    ...
    function estimate_request_list_data() {
        ...
        $list_data = $this->Budgeting_requests_model->get_details($options)->result();
        $result = array();
    ...
    function budgeting_requests_for_client($client_id) {
        $this->access_allowed_members_or_client_contact($client_id);

        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("clients/estimates/budgeting_requests", $view_data);
        }
    }

    // list of estimate requests of a specific client, prepared for datatable 
    function budgeting_requests_list_data_of_client($client_id) {
        $this->access_allowed_members_or_client_contact($client_id);
    ...
    private function _make_estimate_request_row($data) {
        ...
        $edit = '<li role="presentation">' . modal_anchor(get_uri("budgeting_requests/edit_estimate_request_modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("title" => lang('estimate_request'), "data-post-view" => "details", "data-post-id" => $data->id)) . '</li>';

        $request_status = $this->load->view("budgeting_requests/estimate_request_status_options", array("model_info" => $data), true);
        ...
        $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("budgeting_requests/delete_estimate_request"), "data-action" => "delete-confirmation")) . '</li>';
        ...
        return array(
            anchor(get_uri("budgeting_requests/view_estimate_request/" . $data->id), lang("estimate_request") . " - " . $data->id),
            $client,
            $data->form_title,
            $assigned_to,
            $data->created_at,
    ...
    private function _estimate_request_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Budgeting_requests_model->get_details($options)->row();
    ...
    function delete_estimate_request() {
        ...
        if ($this->Budgeting_requests_model->delete($id)) {
    ...
    function estimate_request_filed_list_data($id = 0) {
        ...
        $model_info = $this->Budgeting_requests_model->get_one($id);
    ...
    function estimate_forms() {
        ...
        $this->template->rander('budgeting_requests/estimate_forms');
    }
    ...
    function estimate_request_modal_form() {
        ...
        $this->load->view('budgeting_requests/estimate_request_modal_form', $view_data);
    }
    ...
    private function _make_form_row($data) {
        $title = anchor(get_uri("budgeting_requests/edit_estimate_form/" . $data->id), $data->title, array("class" => "edit", "title" => lang('edit_estimate_form'), "data-post-id" => $data->id));
        ...
        return array(
            ...
            $public,
            lang($data->status),
            modal_anchor(get_uri("budgeting_requests/estimate_request_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_form'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("budgeting_requests/delete_estimate_request_form"), "data-action" => "delete"))
        );
    }
    ...
    function edit_estimate_form($id = 0) {
        ...
        $view_data['model_info'] = $model_info;
        $this->template->rander('budgeting_requests/edit_estimate_form', $view_data);
    }
    ...
    function edit_estimate_request_modal_form() {
        ...
        $model_info = $this->Budgeting_requests_model->get_one($this->input->post("id"));
        //prepare assign to list
        $assigned_to_dropdown = array("" => "-") + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "staff"));
        $view_data['assigned_to_dropdown'] = $assigned_to_dropdown;
        $view_data['model_info'] = $model_info;

        $this->load->view('budgeting_requests/edit_estimate_request_modal_form', $view_data);
    }
    ...
    function update_estimate_request() {
        ...
        $save_id = $this->Budgeting_requests_model->save($data, $id);
        if ($save_id) {
    ...
    function preview_estimate_form($id = 0) {
        ...
        $model_info = $this->Estimate_forms_model->get_one($id);
        $view_data['model_info'] = $model_info;
        $this->template->rander('budgeting_requests/preview_estimate_form', $view_data);
    }
    ...
    function estimate_form_field_modal_form($estimate_form_id = 0) {
        ...
        $view_data['related_to'] = "estimate_request";
        $view_data['estimate_form_id'] = $estimate_form_id;
        $this->load->view('budgeting_requests/estimate_form_field_modal_form', $view_data);
    }
    ...
    private function _make_form_field_row($data) {
        ...
        return array(
            $field,
            $data->sort,
            modal_anchor(get_uri("budgeting_requests/estimate_form_field_modal_form/" . $estimate_form_id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_form'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("budgeting_requests/estimate_form_field_delete"), "data-action" => "delete"))
        );
    }
    ...
    function request_an_estimate_modal_form() {
        ...
        $this->load->view("budgeting_requests/request_an_estimate_modal_form", $view_data);
    }
    ...
    function submit_estimate_request_form($id = 0) {
        ...
        if (get_setting("module_estimate_request") && $model_info->id) {
            ...
            $this->template->rander('budgeting_requests/submit_estimate_request_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function save_estimate_request() {
        ...
        $save_id = $this->Budgeting_requests_model->save($request_data);
        if ($save_id) {
