...
class Purchase_orders extends MY_Controller {
    ...
    private function can_approve_payment() {
        if ($this->login_user->user_type == "staff") {
    ...
    function modal_form() {
        ...
        $labels = explode(",", $this->Purchase_orders_model->get_label_suggestions());
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Purchase_orders_model->delete($id, true)) {
    ...
    function purchase_orders_list_data_of_project($project_id) {
        $options = array(
    ...
    private function _make_row($data) {
        ...
        $row_data = array($purchase_order_url,
            ...
            to_currency($data->purchase_order_value + $data->tax_value, $data->currency_symbol),
            to_currency($data->payment_received, $data->currency_symbol),
            to_currency($data->tax_value, $data->currency_symbol),
            $this->_get_shipment_status_label($data).$this->_get_purchase_order_status_label($data). $this->_get_approval_status_label($data). $purchase_order_labels
        );
    ...
    private function _make_options_dropdown($purchase_order_id = 0) {
        $status = $this->Purchase_orders_model->get_one($purchase_order_id)->approval_status;
        $edit = "";
        $delete = "";
        $add_payment = "";
        if($this->can_edit() && $status == "not_approved") {
        $edit = '<li role="presentation">' . modal_anchor(get_uri("purchase_orders/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("title" => lang('edit_purchase_order'), "data-post-id" => $purchase_order_id)) . '</li>';}

        if($this->can_delete() && $status == "not_approved") {
        $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete_purchase_order'), "class" => "delete", "data-id" => $purchase_order_id, "data-action-url" => get_uri("purchase_orders/delete"), "data-action" => "delete")) . '</li>';}

        if($this->can_add_payment()) {
        $add_payment = '<li role="presentation">' . modal_anchor(get_uri("purchase_order_payments/payment_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment'), array("title" => lang('add_payment'), "data-post-purchase_order_id" => $purchase_order_id)) . '</li>';}
    ...
    function view($purchase_order_id = 0) {
        ...
        if ($purchase_order_id) {
            ...
            if ($view_data) {
                ...
                $view_data["can_approve_payment"] = $this->can_approve_payment();
                $view_data["can_add_payment"] = $this->can_add_payment();
    ...
    private function _get_purchase_order_total_view($purchase_order_id = 0) {
        ...
        $purchase_order_info = $this->Purchase_orders_model->get_details(array("id"=>$purchase_order_id))->row();
    ...
    function save_item() {
        ...
        $purchase_order_item_data = array(
            ...
            "rate" => unformat_currency($this->input->post('purchase_order_item_cost')),
            "total" => $cost * $quantity,
    ...
    function items_modal_form() {
        ...
        $this->load->view('purchase_orders/items_modal_form', $view_data);
    }
    ...
    function save_items() {
        ...
        foreach ($items as $item) {
            ...
            $purchase_order_item_id = $this->Purchase_order_items_model->save($purchase_order_item_data);
        }
    ...
    function delete_item() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Purchase_order_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Purchase_order_items_model->get_details($options)->row();
                echo json_encode(array("success" => true, "purchase_order_id" => $item_info->purchase_order_id, "data" => $this->_make_item_row($item_info), "purchase_order_total_view" => $this->_get_purchase_order_total_view($item_info->purchase_order_id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
    ...
    private function _make_item_row($data) {
        ...
        $row =  array(
            ...
            modal_anchor(get_uri("purchase_orders/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_purchase_order'), "data-post-id" => $data->id))
    ...
    function change_item_qty($operation = 'plus') {
        ...
        if ($this->Purchase_order_items_model->save($data, $id)) {
            $item_info = $this->Purchase_order_items_model->get_one($id);
            echo json_encode(array("success" => true, "quantity" => $quantity, "total" => to_currency($item_info->total, $sym), "purchase_order_total_view" => $this->_get_purchase_order_total_view($item_info->purchase_order_id), 'message' => lang('record_saved')));
        }
    }
    ...
    function update_item_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Purchase_order_items_model->save($data, $id);
            }
        }
    }
    ...
    function get_purchase_order_item_info_suggestion() {
        $item = $this->Purchase_order_items_model->get_item_info_suggestion($this->input->post("item_name"));
        if ($item) {
    ...
    function preview($purchase_order_id = 0, $show_close_preview = false) {
        if ($purchase_order_id) {
            ...
            $view_data['purchase_order_preview'] = prepare_purchase_order_pdf($view_data, "html");
    ...
    function print_purchase_order($purchase_order_id = 0) {
        if ($purchase_order_id) {
            $view_data = get_purchase_order_making_data($purchase_order_id);
            ...
            echo json_encode(array("success" => true, "print_view" => $this->load->view("purchase_orders/print_purchase_order", $view_data, true)));
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
    }
    ...
    function update_purchase_order_status($purchase_order_id = 0, $status = "") {

        if ($purchase_order_id && $status) {
            //change the draft status of the purchase_order
            $this->Purchase_orders_model->update_purchase_order_status($purchase_order_id, $status);
    ...
    function update_approval_status($purchase_order_id = 0, $status = "") {
        //$this->access_only_allowed_members();

        if ($purchase_order_id && $status) {
            //change the draft status of the invoice
            
            $total_summary = $this->Purchase_orders_model->get_purchase_order_total_summary($purchase_order_id);
            if (($status == "approved" || $status == "request_approval") && ($total_summary->balance_due < 0)) {
               ...
               $purchase_order_status = $this->Purchase_orders_model->update_approval_status($purchase_order_id, $status);
               if ($purchase_order_status) {
                $info = $this->Purchase_orders_model->get_details(array("id"=>$purchase_order_status))->row();
                            if ($status == "approved") {
    ...
    function make_po_entries($info) {
        ...
        $type = "Purchase Order: ". get_purchase_order_id($info->id);
        
        $acc_array = array();
        $narration = "Purchase Order: ". get_purchase_order_id($info->id);

        $total_summary = $this->Purchase_orders_model->get_purchase_order_total_summary($info->id);
        ...
        $acc_array[] = array("account_id" => get_setting('default_git'), "type" => 'dr',"amount" => $total_summary->purchase_order_subtotal, "narration" => $narration);
    ...
    function pushes($info) {   
        ...
        $po = get_purchase_order_id($info->id);
        $body = $po.' you created has been approved';
        send_onesignal($body, '#', $notifationTo, '', '');
    }

    function make_prepayment_to_payable($info) {
       $payments = $this->Purchase_order_payments_model->get_all_where(array("purchase_order_id"=> $info->id,"status"=>"approved","deleted"=>0))->result();
       foreach ($payments as $payment) {

            $date = $payment->payment_date;
            $type = "Automatic Prepayment to Supplier Account: ". get_purchase_order_id($payment->purchase_order_id);
