        ...
        if ($signup_key) {
            ...
            if ($valid_key) {
                ...
                if ($type === "staff") {
                    ...
                    if ($user_id) {
                        ////
                        $pt_account_id = $this->Users_model->get_details(array("id"=>$user_id))->row()->pt_account_id;
                        $pt_account = generate_accounts($first_name." ".$last_name, 23, 1, "", $pt_account_id);
                        if ($pt_account_id !== 0) {
                            $pt_account_data = array("pt_account_id" => $pt_account);
                           $this->db->update("users", $pt_account_data, array("id"=>$user_id));
                        }
                        ////
                        $sal_account_id = $this->Users_model->get_details(array("id"=>$user_id))->row()->sal_account_id;
                        $sal_account = generate_accounts($first_name." ".$last_name, 26, 1, "", $sal_account_id);
                        if ($sal_account_id !== 0) {
                            $sal_account_data = array("sal_account_id" => $sal_account);
                            $this->db->update("users", $sal_account_data, array("id"=>$user_id));
                        }
                        //// imbursement acc
                        $imb_account_id = $this->Users_model->get_details(array("id"=>$user_id))->row()->imb_account_id;
                        $imb_account = generate_accounts($first_name." ".$last_name, 48, 1, "", $imb_account_id);
                        if ($imb_account_id !== 0) {
                            $imb_account_data = array("imb_account_id" => $imb_account);
                            $this->db->update("users", $imb_account_data, array("id"=>$user_id));
                        }
                        ////
            ...
            if ($client_id) {
                ////
                $account_id = $this->Clients_model->get_one($client_id)->account_id;
                $account = generate_accounts($company_name, 20, 1, "", $account_id);
                if ($account_id !== 0) {
                    $account_data = array("account_id" => $account);
                   $this->Clients_model->save($account_data, $client_id);
                }
                ////
