...
class Signin extends CI_Controller {
    ...
    function authenticate($email) {
        ...
        if (get_setting("re_captcha_secret_key")){
            ...
            if(!$response){
            $this->form_validation->set_message('authenticate', lang("recaptcha error!"));
            return false;}
        }
        ...
        //IF DEMO SAVE LOGIN EVENT
        if($this->db->dbprefix == 'erp_demo' || $this->db->dbprefix == 'new_erp_demo'){
           //SAVE EVENT
           $data = array(
            "login_date" => get_current_utc_time()
        );
            $this->Demo_login_events_model->save($data);
        } 
