...
class Proforma_invoices extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Proforma_invoices_model');
    ...
    private function can_depart() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "departments") == "all") {
                return true;
            }
        }
    }
    ...
    function index() {
        ...
        $view_data['show_excel'] = isset($this->show_excel)?$this->show_excel:null;
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            if($this->input->post("is_widget"))
            {
                ...
                $this->load->view("proforma_invoices/index", $view_data);
            }
            else
            {

                $this->template->rander("proforma_invoices/index", $view_data);
            }
        } else {
            if (!$this->login_user->is_admin) {
                $view_data['can_create_module'] = get_array_value($this->login_user->permissions,'can_create_invoices');
    ...
    function yearly() {
        $this->load->view("proforma_invoices/yearly_invoices");
    }

    //load the recurring view of invoice list 
    function recurring() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        $this->load->view("proforma_invoices/recurring_invoices_list", $view_data);
    }

    //load the custom view of invoice list 
    function custom() {
        $this->load->view("proforma_invoices/custom_invoices_list");
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Proforma_invoices_model->get_one($this->input->post('id'));


        $client_info = $this->Clients_model->get_one($client_id);
        $client_owner = $client_info->owner_id ? $client_info->owner_id : $this->login_user->id;

        $view_data['client_owner'] = $client_owner;
        ...
        if ($estimate_id) {
            ...
            $model_info->department = $estimate_info->department;
        }
        ...
        $view_data['departments'] = array("" => "-") + $this->Departments_model->get_dropdown_list(array("title"));

        $view_data['department'] = $model_info->department;

        $default_department = get_array_value($this->login_user->permissions, "default_department");
        //default for admin 1 
        if ($this->login_user->is_admin) {
            $default_department = "1";
        }
         $view_data['default_department'] = $default_department;
        ...
        $labels = explode(",", $this->Proforma_invoices_model->get_label_suggestions());
        ...
        $view_data["can_depart"] = $this->can_depart();
        ...
        $this->load->view('proforma_invoices/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $invoice_data = array(
            ...
            "department" => $this->input->post('department') ? $this->input->post('department') : 0,
        ...
        $is_estimate = $this->input->post('is_estimate');
        if ($is_estimate) {
            $invoice_data['quotation_id'] = $this->input->post("estimate_id");
            $invoice_data["discount_amount"] = $this->input->post('discount_amount') ? $this->input->post('discount_amount') : 0;
            $invoice_data["discount_amount_type"] = $this->input->post('discount_amount_type') ? $this->input->post('discount_amount_type') : "percentage";
            $invoice_data["discount_type"] = $this->input->post('discount_type') ? $this->input->post('discount_type') : "before_tax";
        }
        ...
        if ($recurring) {
            ...
            if ($id) {
                ...
                if ($this->input->post('next_recurring_date')) { //submitted any recurring date? set it.
                    ...
                    $invoice_info = $this->Proforma_invoices_model->get_one($id);
        ...
        $invoice_id = $this->Proforma_invoices_model->save($invoice_data, $id);
        if ($invoice_id) {

            $client_data = array ("is_lead" => 0);
            $this->Clients_model->save($client_data, $client_id);
            ...
            if ($is_clone && $main_invoice_id) {
                ...
                $invoice_items = $this->Proforma_invoice_items_model->get_all_where(array("invoice_id" => $main_invoice_id, "deleted" => 0))->result();
                ...
                foreach ($invoice_items as $invoice_item) {
                    ...
                    $invoice_item = $this->Proforma_invoice_items_model->save($invoice_item_data);
                }
            } else {
            ...
            if ($estimate_id) {
                ...
                $this->Proforma_invoices_model->save($invoice_status, $invoice_id);
                ////
                $estimate_items = $this->Estimate_items_model->get_details(array("estimate_id" => $estimate_id))->result();
                ...
                foreach ($estimate_items as $data) {
                    ...
                    $this->Proforma_invoice_items_model->save($invoice_item_data);
                }

                $estimate_stat = array("status" => "accepted");
                $this->Estimates_model->save($estimate_stat, $estimate_id);
            }
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Proforma_invoices_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Proforma_invoices_model->delete($id)) {
    ...
    function list_data($widget = 0) {
        ...
        if($widget)
        {
           $options["approval_status"] = "not_approved"; 
        }
        ...
        $list_data = $this->Proforma_invoices_model->get_details($options)->result();
        //print_r($list_data);
        //exit;
        $result = array();
    ...
    function sub_invoices($recurring_invoice_id) {
        ...
        $this->load->view("proforma_invoices/sub_invoices", $view_data);
    }
    ...
    private function _row_data($id) {
        ...
        $data = $this->Proforma_invoices_model->get_details($options)->row();
    ...
    private function _make_row($data, $custom_fields) {
        $invoice_url = "";
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("proforma_invoices/view/" . $data->id), get_advance_invoice_id($data->id));
        } else {
            $invoice_url = anchor(get_uri("proforma_invoices/preview/" . $data->id), get_advance_invoice_id($data->id));
        }
        ...
        $department = $this->Departments_model->get_one($data->department);
        if ($department) {
        ...
        $owner = $this->Users_model->get_one($data->owner);
        if ($owner) {
        ...
        if ($data->lead_source) {
            $source_info = $this->Lead_source_model->get_one($data->lead_source);
            $source = "Source: ".$source_info->title;
        } else {
            $source = "";
        }

        $refunded_payment = $this->Proforma_invoices_model->get_refunded_payments($data->id)->total_paid ? $this->Proforma_invoices_model->get_refunded_payments($data->id)->total_paid: 0;

        $row_data = array($invoice_url."</br>".$department->title,
            anchor(get_uri("clients/view/" . $data->client_id), $data->company_name),
            ($data->owner ? anchor(get_uri("team_members/view/" . $data->owner), $owner_name) : "-")."</br>".$source,
            $data->bill_date,
            format_to_date($data->bill_date, false),
            $data->due_date,
            format_to_date($data->due_date, false),
            to_currency($data->invoice_value, $data->currency_symbol),
            to_currency($data->tax_values, $data->currency_symbol),
            to_currency($data->payment_received - $refunded_payment, $data->currency_symbol),
            to_currency(($data->invoice_value - $data->payment_received + $refunded_payment), $data->currency_symbol),
            "<span> Approval: ".$this->_get_approval_status_label($data)."</span>".
            "<span> Payment: ".$this->_get_invoice_status_label($data)."</span>".
            "<span> Sale: ".$this->_get_delivery_status_label($data). "</span>". $invoice_labels,
            $data->create_user."</br>".$data->create_time,
    ...
    function test($invoice_id = 8)
    {
        ...
        $invoice_total_items = $this->Proforma_invoice_items_model->get_sum_items($invoice_id)->row();
    ...
    private function _make_options_dropdown($invoice_id = 0) {
        $invoice_status = $this->Proforma_invoices_model->get_one($invoice_id)->approval_status;
        $edit = "";
        $delete = "";
        $add_payment = "";
        if($this->can_edit() && $invoice_status == "not_approved") {
        $edit = '<li role="presentation">' . modal_anchor(get_uri("proforma_invoices/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("title" => lang('edit_invoice'), "data-post-id" => $invoice_id)) . '</li>';
        }
        if($this->can_delete() && $invoice_status == "not_approved") {
        $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete_invoice'), "class" => "delete", "data-id" => $invoice_id, "data-action-url" => get_uri("proforma_invoices/delete"), "data-action" => "delete")) . '</li>'; }
    ...
    private function _make_recurring_row($data) {
        ...
        $invoice_url = anchor(get_uri("proforma_invoices/view/" . $data->id), get_invoice_id($data->id));
    ...
    function view($invoice_id = 0) {
        ...
        if ($invoice_id) {
            $this->load->model('Proforma_invoices_model');
            $view_data = get_proforma_invoice_making_data($invoice_id);
            ...
            if ($view_data) {
                ...
                // $view_data["delivery_info"] = $this->Proforma_invoices_model->get_one_where(array("invoice_id"=>$invoice_id, "deleted" => 0));
                $view_data["delivery_info"] = $this->Proforma_invoices_model->get_one_where(array("id"=>$invoice_id, "deleted" => 0));
                $view_data["can_approve"] = $this->can_approve();
                $view_data["can_add_payment"] = $this->can_add_payment();
                $view_data["can_add_do"] = $this->can_add_do();

                /*info for the expense part */
                if (!$this->login_user->is_admin) {
                ...
                $view_data["expense_custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("expenses", $this->login_user->is_admin, $this->login_user->user_type);

                $view_data["can_approve_expense"] = $this->can_approve_expense();

                // Calculate Revenue

                $options = array("invoice_id" => $invoice_id);
                $data = $this->Reports_model->get_invoice_revenue($options)->row();
                // die('hii') ;
                $revenue = $data->invoice_value - ($data->item_cal_cost + $data->expense_value); 

                if ($revenue > 0) {
                    $view_data["class"] = "success";
                } else {
                    $view_data["class"] = "danger";
                }

                $view_data["revenue"] = $revenue;
                $view_data["percentage"] = $data->invoice_value ? $revenue / $data->invoice_value : 0;
...
//  var_dump($view_data);
                $this->template->rander("proforma_invoices/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    private function can_approve_expense() {
        if ($this->login_user->user_type == "staff") {
    ...
    private function _get_invoice_total_view($invoice_id = 0) {
        ...
        $invoice_uncollected_cheques = $this->Reports_model->invoice_uncollected_cheques($invoice_id)->amount ? $this->Reports_model->invoice_uncollected_cheques($invoice_id)->amount : 0;
        $view_data["invoice_uncollected_cheques"] = $invoice_uncollected_cheques;
        return $this->load->view('proforma_invoices/invoice_total_section', $view_data, true);
    }

    private function can_edit_rate($item_type = "") {
        if ($this->login_user->user_type == "staff") {
    ...
    function item_modal_form() {
        ...
        $view_data['model_info'] = $this->Proforma_invoice_items_model->get_one($this->input->post('id'));
        if (!$invoice_id) {
        ...
        $view_data['types_dropdown'] = array("" => "-", 'service'=>lang('service'));
        $view_data['levels_dropdown'] = array("" => "-") + $this->Items_levels_model->get_dropdown_list(array("level_name"));
        $view_data['invoice_id'] = $invoice_id;

        $do_quantity =  1;
        if ($this->Proforma_invoices_model->get_details(array("id" => $invoice_id))->row()->delivery_status == "delivered" && $this->input->post('id')) {
        $do_qty = $this->Proforma_invoice_items_model->get_delivered_items($this->input->post('id'));
        $do_quantity =  isset($do_qty) ? $do_qty : 1;
        }

        $view_data['min_qty'] = $do_quantity;

    
        $item_type = "";
        $view_data['item_full_name'] = "";
        $item = $this->input->post('id');
        if ($item) {
            $item_id = $view_data['model_info']->item_id;
            $item_type = $this->Items_model->get_one($item_id)->item_type; 
            $view_data['item_full_name'] = $this->Items_model->get_one($item_id)->full_name;
        }

        $view_data['can_edit_rate'] = $this->can_edit_rate($item_type);

         $view_data['taxes_dropdown'] = $this->Taxes_model->get_dropdown_list(array("title")) + array("" => "-");

        $this->load->view('proforma_invoices/item_modal_form', $view_data);
    }
    ...
    function save_item() {
        ...
        $invoice_item_id = $this->Proforma_invoice_items_model->save($invoice_item_data, $id);
    //    echo $invoice_item_id;die();
        if ($invoice_item_id) {
            ...
            if ($add_new_item_to_library) {
                $library_item_data = array(
                    ...
                    "cost" => unformat_currency($this->input->post('invoice_item_cost')),
                    "item_level" => $this->input->post('item_level')
                );
                $item_save =$this->Items_model->save($library_item_data);
                if($item_save) {
                    $item_id_data = array("item_id" => $item_save);
                    $this->Proforma_invoice_items_model->save($item_id_data, $invoice_item_id);
                }
            }

            $options = array("id" => $invoice_item_id);
            $item_info = $this->Proforma_invoice_items_model->get_details($options)->row();
    ...
    function items_modal_form() {
        ...
        // $this->levels_checker("items_levels");
        $items_levels = $this->restrictions;

        $items = $this->Proforma_invoice_items_model->get_items($items_levels);
        $items_dropdown = array();

        foreach ($items as $item) {
            $item_level_ = $this->Items_levels_model->get_one($item->item_level)->level_name;
            $items_dropdown[] = array("id" => $item->id, "text" => $item->title." - ".$item_level_);
        }

        $view_data['items_dropdown'] = json_encode($items_dropdown);

        $this->load->view('proforma_invoices/items_modal_form', $view_data);
    }
    ...
    function save_items() {
        ...
        foreach ($items as $item) {
            ...
            $invoice_item_data = array(
                ...
                "tax_id" => $this->input->post('tax_id') ? $this->input->post('tax_id') : 1,
            );

            $invoice_item_id = $this->Proforma_invoice_items_model->save($invoice_item_data);
            if ($invoice_item_id) {
                $options = array("id" => $invoice_item_id);
                $item_info = $this->Proforma_invoice_items_model->get_details($options)->row();
                $data[] = array(
    ...
    function delete_item() {
        ...
        $id = $this->input->post('id');
        if ($this->input->post('undo')) {
            if ($this->Proforma_invoice_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Proforma_invoice_items_model->get_details($options)->row();
            ...
            if ($this->Proforma_invoice_items_model->delete($id)) {
                ...
                $item_info = $this->Proforma_invoice_items_model->get_one($id);
    ...
    function item_list_data($invoice_id = 0) {
        ...
        $list_data = $this->Proforma_invoice_items_model->get_details(array("invoice_id" => $invoice_id))->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_item_row($data) {
        ...
        //get the real name of the item : Amar
        // if ($this->login_user->is_admin) {
        //     $real_name = $this->Items_model->get_one($data->item_id)->full_name?$this->Items_model->get_one($data->item_id)->full_name:'-';
        //     $item .= "<br /><span style='color:red'>" . $real_name . "</span>";
        // }
        ...
        $do_quantity =  0;
        $do_type = "";
        if ($this->Proforma_invoices_model->get_details(array("id" => $data->invoice_id))->row()->delivery_status == "delivered") {
        $do_item = $this->Proforma_invoice_items_model->get_details(array("invoice_item_id" => $data->id))->row();
        $do_qty = $this->Proforma_invoice_items_model->get_delivered_items($data->id);
        $do_type = isset($do_item->unit_type) ? ($do_item->unit_type ? $do_item->unit_type : "") : "";
        $do_quantity =  isset($do_qty) ? $do_qty : 0;
        }

        $edit_button = ($this->login_user->is_admin == "1" || get_array_value($this->login_user->permissions,'edit_items_in') == "all") ? modal_anchor(get_uri("proforma_invoices/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id)): "";
        $delete_button = ($do_quantity == 0) ? js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("proforma_invoices/delete_item"), "data-action" => "delete")): "";
        ...
        return array(
            ...
            $edit_button
            . $delete_button
            . js_anchor("<i class='fa fa-minus fa-fw'></i>", array('title' => lang('substract'), "class" => "delete", "data-id" => $data->id, "data-qty" => $data->quantity, "data-action-url" => get_uri("proforma_invoices/change_item_qty/minus"), "data-action" => "qty"))
            . js_anchor("<i class='fa fa-plus fa-fw'></i>", array('title' => lang('add'), "class" => "edit", "data-id" => $data->id, "data-qty" => $data->quantity,"data-action-url" => get_uri("proforma_invoices/change_item_qty/plus"), "data-action" => "qty"))
        );
    }
    ...
    function change_item_qty($operation = 'plus') {
        ...
        $do_quantity =  1;
        if ($this->Proforma_invoices_model->get_details(array("id" => $item_info->invoice_id))->row()->delivery_status == "delivered") {
        $do_qty = $this->Proforma_invoice_items_model->get_delivered_items($id);
        $do_quantity =  isset($do_qty) ? $do_qty : 1;
        }
        ...
        if ($this->Proforma_invoice_items_model->save($data, $id)) {
            $item_info = $this->Proforma_invoice_items_model->get_one($id);
    ...
    function update_item_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Proforma_invoice_items_model->save($data, $id);
            }
        }
    }
    ...
    function get_invoice_item_suggestion() {
        ...
        $items = $this->Proforma_invoice_items_model->get_item_suggestion($key, 0);
    ...
    function get_invoice_item_info_suggestion() {
        $item = $this->Proforma_invoice_items_model->get_item_info_suggestion($this->input->post("item_name"));
        if ($item) {
    ...
    function preview($invoice_id = 0, $show_close_preview = false) {
        if ($invoice_id) {
            ...
            $this->template->rander("proforma_invoices/invoice_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function print_invoice($invoice_id = 0) {
        if ($invoice_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->load->view("proforma_invoices/print_invoice", $view_data, true)));
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
    }
    ...
    function download_pdf($invoice_id = 0) {
        ...
        if ($invoice_id) {
            ...
            $invoice_data["client_info"]->lead_source_id = 0;
            $invoice_data["client_info"]->owner_id = 0;
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Proforma_invoices_model->get_details($options)->row();
            ...
            $invoice_total_summary = $this->Proforma_invoices_model->get_invoice_total_summary($invoice_id);
            ...
            $parser_data["INVOICE_URL"] = get_uri("proforma_invoices/preview/" . $invoice_info->id);
            ...
            $this->load->view('proforma_invoices/send_invoice_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function send_invoice() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => array(array("file_path" => $attachement_url)), "cc" => $cc, "bcc" => $bcc_emails))) {
            ...
            if ($this->Proforma_invoices_model->save($status_data, $invoice_id)) {
    ...
    function get_invoice_status_bar($invoice_id = 0) {
        ...
        $view_data["invoice_info"] = $this->Proforma_invoices_model->get_details(array("id" => $invoice_id))->row();
        ...
        $view_data["delivery"] = $this->_get_delivery_status_label($view_data["invoice_info"]);
         ...
         // Calculate Revenue
                ...
                $view_data["revenue"] = $revenue;
                $view_data["percentage"] = $revenue / $data->invoice_value; 

        $this->load->view('proforma_invoices/invoice_status_bar', $view_data);
    ...
    function update_invoice_status($invoice_id = 0, $status = "") {
        ...
        if ($invoice_id && $status) {
            ...
            $this->Proforma_invoices_model->update_invoice_status($invoice_id, $status);
            ...
            if ($status == "cancelled") {
                ...
                $this->Proforma_invoices_model->save($data, $invoice_id);
            }
    ...
    function update_delivery_status($invoice_id = 0, $status = "") {
        ...
        if ($invoice_id && $status) {
            //change the draft status of the invoice
            $this->Proforma_invoices_model->update_delivery_status($invoice_id, $status);
    ...
    function discount_modal_form() {
        ...
        $invoice_id = $this->input->post('invoice_id');
        $data = $this->Proforma_invoices_model->get_invoice_total_summary($invoice_id);
        ...
        $view_data['model_info'] = $this->Proforma_invoices_model->get_one($invoice_id);

        $this->load->view('proforma_invoices/discount_modal_form', $view_data);
    }
    ...
    function save_discount() {
        ...
        if ($this->input->post('discount_amount_type') == "percentage") {
            ...
            validate_submitted_data(array(
                "invoice_id" => "required|numeric",
                //"discount_type" => "required",
                "discount_amount" => "numeric|less_than[$rr]",
                "discount_amount_type" => "required"
            ));
        ...
        $data = array(
            "discount_type" => $this->input->post('discount_type') ? $this->input->post('discount_type'): "after_tax",
        ...
        $save_data = $this->Proforma_invoices_model->save($data, $invoice_id);
        if ($save_data) {
    ...
    function update_approval_status($invoice_id = 0, $status = "") {
        ...
        if ($invoice_id && $status) {
            //change the draft status of the invoice
            $total_summary = $this->Proforma_invoices_model->get_invoice_total_summary($invoice_id);
            /*if (($status == "approved" || $status == "request_approval") && ($total_summary->balance_due < 0)) {
                $message = 'balance_due_can_not_be_a_negative_value';
                echo json_encode(array("success" => false, 'message' => $message)); 
            } else {*/
                $revised = 0;
                $invoice_status = $this->Proforma_invoices_model->update_approval_status($invoice_id, $status);
                if ($invoice_status) {
                    $info = $this->Proforma_invoices_model->get_details(array("id"=>$invoice_status))->row();
                    if ($status == "approved") {
                        $this->pushes($info);
                    } else {
                        // Ammar: update the revised status
                        $revised = intval($total_summary->revised) + 1;
                        $data = array("revised" => $revised);
                        $save = $this->Proforma_invoices_model->save($data, $invoice_id);

                    }
                   echo json_encode(array("success" => true, 'message' => lang('record_saved'), "revised" => $revised)); 
                }
            /*}*/
    ...
    function save_note(){
        validate_submitted_data(array(
            "invoice_note_id" => "required|numeric"
        ));

        $id = $this->input->post('invoice_note_id');

        $data = array(
            "note" => decode_ajax_post_data($this->input->post('note_textarea'))
        );
        $save_id = $this->Proforma_invoices_model->save($data, $id);
        if ($save_id) {

            $invoice_info = $this->Proforma_invoices_model->get_one($save_id);
            echo json_encode(array("success" => true, 'id' => $save_id, 'message' => lang('record_saved'), "note" => $invoice_info->note));
        } else {
            echo json_encode(array("success" => false, 'message' => lang('error_occurred')));
        }
    }
