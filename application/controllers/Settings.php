...
class Settings extends MY_Controller {
    ...
    function save_general_settings() {
        $settings = array("site_logo", "favicon", "show_background_image_in_signin_page", "show_logo_in_signin_page", "app_title", "language", "timezone", "date_format", "time_format", "first_day_of_week","weekend", "default_currency", "currency_symbol", "currency_position", "decimal_separator", "no_of_decimals", "accepted_file_formats", "rows_per_page", "item_purchase_code", "scrollbar", "enable_rich_text_editor");
    ...
    function save_invoice_settings() {
        $settings = array(/*"allow_partial_invoice_payment_from_clients",*/ "invoice_footer", "send_bcc_to", "invoice_prefix", /*"invoice_style",*/ "invoice_color", "invoice_logo", /*"send_invoice_due_pre_reminder", "send_invoice_due_after_reminder", "send_recurring_invoice_reminder_before_creation",*/ "default_due_date_after_billing_date", "initial_number_of_the_invoice");
    ...
    function attendance() {
        $this->template->rander("settings/attendance");
    }

    function save_attendance_password() {
        $this->Settings_model->save_setting("attendance_password", $this->input->post("attendance_password"));
        $this->Settings_model->save_setting("enable_attendance", $this->input->post("enable_attendance"));
    ...
    function accounting_settings() {
        ...
        foreach ($accounts as $account) {
            $accounts_dropdown[] = array("id" => $account->id, "text" => $account->acc_code." ".$account->acc_name);
        }

        $view_data['accounts_dropdown'] = json_encode($accounts_dropdown);
        $this->template->rander("settings/accounting", $view_data);
    }

    function save_accounting_settings() {
        // $settings = array("financial_year_end", "default_bank", "clients_accounts_parent","suppliers_accounts_parent","banks_accounts_parent","cash_on_hand_accounts_parent","expenses_accounts_parent","default_cash_on_hand", "payable_cheques", "receivable_cheques", "default_inventory", "cost_of_goods_sold", "discount","default_sales","VAT_in", "VAT_out", "VAT_expense",'salary_expenses', 'leave_salary_expenses', 'payable_salaries', 'payable_leave_salaries', 'payable_EOS_benefits', 'payable_PASI', 'payable_job_security', 'salary_advances');
        $settings = array( "clients_accounts_parent","suppliers_accounts_parent","banks_accounts_parent","cash_on_hand_accounts_parent","expenses_accounts_parent","petty_cash_parent", "default_git","payable_cheques", "receivable_cheques", "default_inventory",  "discount","VAT_in", "VAT_out",'salary_expenses',  'payable_salaries',  'payable_PASI',  'salary_advances');
