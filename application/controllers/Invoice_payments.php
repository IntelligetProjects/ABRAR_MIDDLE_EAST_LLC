...
class Invoice_payments extends MY_Controller {
    ...
    function payment_modal_form() {
        ...
        if (!$invoice_id) {
            ...
            $view_data["balance_due"] = $this->Invoices_model->get_invoice_total_summary($invoice_id)->balance_due;
        }
    ...
    private function _make_payment_row($data) {
        ...
        if ($this->can_edit() && $data->status == "draft") {
            $rowe .= modal_anchor(get_uri("invoice_payments/payment_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_payment'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id,));
        }
        if ($this->can_delete() && $data->status == "draft") {
            $rowe .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("invoice_payments/delete_payment"), "data-action" => "delete"));
        }
        $row[] = $rowe;
        return $row;
    ...
    function update_status ($stat = '', $sent_id = 0){
        ...
        $status_id = $this->Invoice_payments_model->save($data, $id);
        if ($status_id) {
            echo json_encode(array("success" => true, 'message' => 'success'));
            $info = $this->Invoice_payments_model->get_details(array("id" => $status_id))->row();
    ...
    function make_entries($info) {
        ...
        $type = "Invoice Payment Received: ". get_invoice_id($info->invoice_id);
        ...
        $inv_info = $this->Invoices_model->get_one($info->invoice_id);

        /*if($inv_info->approval_status == 'approved') {*/
        ...
        if ($info->payment_method_id == 1) {
            ...
            // $acc_array[] = array("account_id" => get_setting('default_cash_on_hand'), "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
            $acc_array[] = array("account_id" => $info->treasury, "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
            
        } else if ($info->payment_method_id == 4) {
            $acc_array[] = array("account_id" => $account_id, "type" => 'cr',"amount" => $info->amount, "narration" => $narration);

            // $acc_array[] = array("account_id" => get_setting('default_bank'), "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
            $acc_array[] = array("account_id" => $info->bank, "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
            
        } else if ($info->payment_method_id == 5) {
            $acc_array[] = array("account_id" => $account_id, "type" => 'cr',"amount" => $info->amount, "narration" => $narration);

            $acc_array[] = array("account_id" => get_setting('receivable_cheques'), "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
            // $acc_array[] = array("account_id" => $info->bank, "type" => 'dr',"amount" => $info->amount, "narration" => $narration);
        ...
        if ($transaction_id) {
            $trans_data = array("account_transaction_id" => $transaction_id);
    ...
    function delete_entries($info) {   
        ...
        if ($trans_id !== 0) {
            ...
            if ($transaction_id) {
                $trans_data = array("account_transaction_id" => 0, "advance_transaction_id" => 0);
                $this->Invoice_payments_model->save($trans_data, $info->id);
            }
        }
        ...
        if ($cheque_trans_id !== 0) {
            ...
            if ($cheque_transaction_id) {
                ...
                $this->Invoice_payments_model->save($che_trans_data, $info->id);
            }
        }
    }
