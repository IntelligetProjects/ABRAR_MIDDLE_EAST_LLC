...
class Expense_categories extends MY_Controller {
    ...
    function index($accounting = 0) {
        ...
        $this->template->rander("expense_categories/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data['category_parent'] = array("" => "-") + $this->Accounts_model->get_dropdown_list(array("acc_name"), "id", array("acc_parent" => get_setting('expenses_accounts_parent'), "deleted" => 0));
    ...
    function save() {
        ...
        if ($save_id) {
            ////
            $account_id = $this->Expense_categories_model->get_one($save_id)->account_id;
            // $account = generate_accounts($this->input->post('title'),  $this->Accounts_model->get_expense_accounts_id(), $account_id);
            $account = generate_accounts($this->input->post('title'),  $this->input->post('category_parent'), $account_id);
            if ($account_id !== 0) {
                $account_data = array("account_id" => $account);
               $this->Expense_categories_model->save($account_data, $save_id);
            }
            ////
