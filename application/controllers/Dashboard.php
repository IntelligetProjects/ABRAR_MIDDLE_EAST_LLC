...
class Dashboard extends MY_Controller {
    ...
    public function index() {
        ...
        if ($this->login_user->user_type === "staff" && $this->show_staff_on_staff) {
            ...
            $view_data['content_view'] = "dashboards/arkiton_index";
    ...
    private function _check_widgets_for_staffs() {
        ...
        $access_expense = $this->get_access("can_access_expenses");
        $access_invoice = $this->get_access("can_access_invoices");
        $access_purchase_order = $this->get_access("can_access_purchase_orders");
        $access_estimate = $this->get_access("can_access_estimates");
        $access_client = $this->get_access("can_access_clients");
        $access_contact = $this->get_access("can_access_contacts");
        $access_supplier = $this->get_access("can_access_suppliers");
        $access_invoice_payment = $this->get_access("can_access_invoice_payments");
        $access_purchase_order_payment = $this->get_access("can_access_purchase_order_payments");

        $access_eroom = $this->get_access_info("eroom");
        $access_report = $this->get_access_info("reports");
        $access_lead = $this->get_access_info("lead_permission");
        $access_estimate_request = $this->get_access_info("estimate_requests");
        $access_payroll = $this->get_access_info("payroll");
        ...
        if ($show_invoice && $show_expense && $access_expense->access_type === "1" && $access_invoice->access_type === "1") {
            $widget["income_vs_expenses"] = true;
        }

        if ($show_invoice && $access_invoice->access_type === "1") {
        ...
        if ($show_invoice && ($this->login_user->is_admin || $access_invoice->access_type == "1")) {
            $widget["total_invoices"] = true;
            $widget["total_payments"] = true;
        }

        if ($show_expense && $show_invoice && $access_invoice->access_type == "1") {
        ...
        //////
        if ($show_invoice && $access_invoice->access_type === "1") {
        $widget["invoices"] = true;
        }
        if ($access_client->access_type === "1") {
        $widget["clients"] = true;
        }
        if($access_estimate->access_type === "1") {$widget["estimates"] = true;}
        

        if($access_expense->access_type === "1") {$widget["expenses"] = true;}
        if($access_invoice_payment->access_type === "1") {$widget["invoice_payments"] = true;}
        if($access_invoice->access_type === "1") {$widget["delivery_notes"] = true;}
        if($access_purchase_order_payment->access_type === "1") {$widget["items"] = true;}
        if($access_purchase_order->access_type === "1") {$widget["purchase_orders"] = true;}
        if($access_purchase_order_payment->access_type === "1") {$widget["purchase_order_payments"] = true;}
        if($access_supplier->access_type === "1") {$widget["suppliers"] = true;}
        if($access_expense->access_type === "1") {$widget["PT_cash"] = true;}
        if($access_estimate_request->access_type === "all") {$widget["estimate_requests"] = true;}
        if($access_lead->access_type === "all") {$widget["leads"] = true;}
        if($access_payroll->access_type === "all") {$widget["payroll"] = true;}
        if($access_report->access_type === "all") {$widget["reports"] = true;}
        if($access_eroom->access_type === "all") {$widget["eroom"] = true;}
        if($access_contact->access_type === "1") {$widget["contacts"] = true;}
        if($show_message) {$widget["messages"] = true;}
        if(get_setting("module_timeline")) {$widget["timeline"] = true;}
        if(get_setting("module_todo")) {$widget["to_do"] = true;}
    ...
    public function save_external_links() {
        $external_links = $this->input->post("website1").",".$this->input->post("website2").",".$this->input->post("website3").",".$this->input->post("website4");
        $data = array("external_links" => $external_links);
        $this->Users_model->save($data, $this->login_user->id);

        echo json_encode(array("success" => true, 'message' => lang('record_saved')));
    }

    function links_form($id = 0) {
        $this->load->view("dashboards/custom_widgets/links_form");
    }
    ...
    private function _get_widgets_for_staffs($widget, $widgets_array) {
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == "clock_in_out") {
                ...
                return internal_links(true, "leads");
            } else if ($widget == "estimate_requests") {
                return internal_links(true, "estimate_requests");
            } else if ($widget == "eroom") {
                return internal_links(true, "eroom");
            } else if ($widget == "contacts") {
                return internal_links(true, "contacts");
            } else if ($widget == "payroll") {
                return internal_links(true, "payroll");

            } else if ($widget == "invoices") {
                return internal_links(true, "invoices");
            } else if ($widget == "clients") {
                return internal_links(true, "clients");
            } else if ($widget == "estimates") {
                return internal_links(true, "estimates");
            } else if ($widget == "expenses") {
                return internal_links(true, "expenses");
            } else if ($widget == "invoice_payments") {
                return internal_links(true, "invoice_payments");
            } else if ($widget == "delivery_notes") {
                return internal_links(true, "delivery_notes");
            } else if ($widget == "items") {
                return internal_links(true, "items");
            } else if ($widget == "purchase_orders") {
                return internal_links(true, "purchase_orders");
            } else if ($widget == "purchase_order_payments") {
                return internal_links(true, "purchase_order_payments");
            } else if ($widget == "suppliers") {
                return internal_links(true, "suppliers");
            } else if ($widget == "PT_cash") {
                return internal_links(true, "PT_cash");
            } else if ($widget == "projects") {
                return internal_links(true, "projects");
            } else if ($widget == "messages") {
                return internal_links(true, "messages");
            } else if ($widget == "timeline") {
                return internal_links(true, "timeline");
            } else if ($widget == "notes") {
                return internal_links(true, "notes");
            } else if ($widget == "events_today") {
                return events_today_widget(true);
            } else if ($widget == "external_links") {
                return external_links(true);
