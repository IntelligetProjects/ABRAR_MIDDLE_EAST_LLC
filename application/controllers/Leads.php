...
class Leads extends MY_Controller {
    ...
    function index() {
        $this->p_access_check("can_access_leads");
        ...
        if (!$this->login_user->is_admin) {
            $view_data['can_create_module'] = get_array_value($this->login_user->permissions,'can_create_leads');
        } else {
    ...
    function save() {
        // $this->access_only_allowed_members();
       if($this->p_check("can_create_leads")!=true){
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if($this->p_check("can_edit_leads") && $this->p_check("can_delete_leads")){
        ...
        }elseif($this->p_check("can_edit_leads")){
            $row_data[] = modal_anchor(get_uri("leads/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_lead'), "data-post-id" => $data->id));
        }elseif($this->p_check("can_delete_leads")){
            $row_data[] =js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_lead'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("leads/delete"), "data-action" => "delete-confirmation")) ;
        }else{
            $row_data[] =''; 
        }
    ...
    function save_as_client() {
        ...
        if ($client_id) {
            ...
            if ($save_client_id) {
                ////
                $account_id = $this->Clients_model->get_one($save_client_id)->account_id;
                // $account = generate_accounts($company_name, 20, 1, "", $account_id);
                ...
                if ($account_id !== 0) {
                   ...
                   $this->Clients_model->save($account_data, $save_client_id);
                }
                ////
                // $advance_account_id = $this->Clients_model->get_one($save_client_id)->advance_account_id;
                // $advance_account = generate_accounts($company_name, 60, 1, "", $advance_account_id);
                // if ($advance_account_id !== 0) {
                //     $advance_account_data = array("advance_account_id" => $advance_account);
                //    $this->Clients_model->save($advance_account_data, $save_client_id);
                // }
                ////
